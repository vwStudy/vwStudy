...
class AnimationVisualizer:
    ...
    def init(self):
        ...
        return self.car_circles + self.end_circles + self.obstacle_circles + [self.lines]
    ...
    def update(self, frame):
        ...
        # # 各車の終点を確認し、到達したら新しい終点を生成
        # for i in range(len(positions)):
        #     if np.linalg.norm(positions[i] - self.end_positions[i]) < 1e-5:  # 終点に到達したとみなす
        #         self.end_positions[i] = np.random.randint(0, 20, size=2)  # 新しい終点をランダムに生成
    ...
    def animate(self):
        ...
        #print("f", len(self.trajectory))
        ###ここで描写用の車を生成しているから、ifでスタート地点ごとに色を分けれそう
