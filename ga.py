...
def main_two_steps(popu_size, gene_size, genom_size):
    ...
    populations = create_generation(popu_size, genom_size, fitness)
    # print("populations", populations)インスタンスが入ってる1次元リスト
    best =  ga_solve(populations, gene_size)
    ...
    return ga_solve_two_steps(populations, gene_size, two_steps_list, zeros_list)
...
if __name__ == '__main__':
    ...
    with open('./data_folder/individual_twostep.csv', 'a', encoding='utf-8', newline='') as f:
        writer = csv.writer(f)
        for _ in range(4):
            for _ in range(4):
                for i in range(10):
