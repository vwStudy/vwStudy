...
class VW:
    ...
    def create_VW(screen, color, GA_list):
        ...
        for index, oneDivisionList in enumerate(ga_list):
            for twoDivisionIndex, deploy_check in enumerate(oneDivisionList):
                if deploy_check >= 1:
                    ...
                    surface = pygame.Surface((VW_LeftUp[0], VW_LeftUp[1]), pygame.SRCALPHA)
                    
                    pygame.draw.rect(surface, color, pygame.Rect(VW_LeftUp[0], VW_LeftUp[1], abs(VW_RightDown[0]-VW_LeftUp[0]), abs(VW_RightDown[1]-VW_LeftUp[1])))
                    screen.blit(surface, (VW_LeftUp[0], VW_LeftUp[1]))

                    pygame.display.flip()

                    #pygame.draw.rect(screen, color, (VW_LeftUp[0], VW_LeftUp[1], abs(VW_RightDown[0]-VW_LeftUp[0]), abs(VW_RightDown[1]-VW_LeftUp[1]))) #透明じゃないVW
        
        return surface, VW_LeftUp
...
def main():
    ...
    surface, leftup = VW.create_VW(screen, vw_red, car_ga[0])
    surface, leftup = VW.create_VW(screen, vw_blue, car_ga[1])
    surface, leftup = VW.create_VW(screen, vw_green, car_ga[2])
    surface, leftup = VW.create_VW(screen, vw_yellow, car_ga[3])
    ...
    while True:
        ...
        # surface, leftup = VW.create_VW(screen, red, car_ga[0])
        # #screen.blit(surface, (leftup[0], leftup[1]))
        # surface, leftup = VW.create_VW(screen, vw_blue, car_ga[1])
        # #screen.blit(surface, (leftup[0], leftup[1]))
        # surface, leftup = VW.create_VW(screen, vw_green, car_ga[2])
        # #screen.blit(surface, (leftup[0], leftup[1]))
        ...
        VW.create_VW(screen, vw_red, car_ga[0])
        VW.create_VW(screen, vw_blue, car_ga[1])
        VW.create_VW(screen, vw_green, car_ga[2])
        VW.create_VW(screen, vw_yellow, car_ga[3])
