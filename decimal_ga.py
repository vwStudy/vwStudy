...
def ga_solve(populations, gene_size):
    best = []
    generation_list = []
    elite_count = int(len(populations)/4)
    for i in range(gene_size, 0, -1):
        best_popu = min(populations, key=Individual.get_fitness)
        best.append(best_popu)

        elite_individuals = sorted(populations, key=Individual.get_fitness)[:elite_count]
        generation_list.append(populations)
        selected = select_tonament(populations)
            
        children = crossover(selected, populations)
        
        children = mutate(children,gene_size)
        populations = elite_individuals + children[:len(populations) - elite_count]
